...
public class AddAnnonce extends AppCompatActivity {
    ...
    public final String PRODUCT_TABLE = "Announces";
    TextInputEditText Category_view;
    AnnonceModel new_product = new AnnonceModel();
    ...
    private void bind_views() {
        new_product.Annonce_id = db.collection(PRODUCT_TABLE).document().getId();
    ...
    public static final String[] categories = new String[]{"Women", "Men", "Kids", "Electronics","Bricolage","Décoration","Immobilier","Fourniture de bureau", "Montre et Bijoux"};
    ...
    private void upload_to_firestore() {
        ...
        db.collection(PRODUCT_TABLE).document(new_product.Annonce_id).set(new_product).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                progressDialog.hide();
                progressDialog.dismiss();
                Toast.makeText(AddAnnonce.this, "Uploaded successfully!.", Toast.LENGTH_SHORT).show();
                finish();
                return;
            }
            ...
            public void onFailure(@NonNull Exception e) {
                progressDialog.hide();
                Toast.makeText(AddAnnonce.this, "Failed upload product because " + e.getMessage(), Toast.LENGTH_LONG).show();
                return;
            }
        });
    ...
    private void submit_product() {
        ...
        if (new_product.category.isEmpty()) {
            Toast.makeText(this, "vous devriez selectionné une categorie.", Toast.LENGTH_SHORT).show();
            select_category();
            return;
        }
        ...
        if (product_price.getText().toString().isEmpty()) {
            Toast.makeText(this, " le prix ne peut pas etre vide.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (new_product.price < 0) {
            Toast.makeText(this, "le prix ne peut pas etre moins de zero.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        storage_ref_main.child("Annonces/" + new_product.Annonce_id).putFile(imagePath).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                Toast.makeText(AddAnnonce.this, "Uploaded Successfully!", Toast.LENGTH_SHORT).show();

                storage_ref_main.child("products/" + new_product.Annonce_id).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
            ...
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(AddAnnonce.this, "Failed to upload photo" + e.getMessage(), Toast.LENGTH_SHORT).show();
                new_product.photo = "https://images.unsplash.com/photo-1491553895911-0055eca6402d?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&auto=format&fit=crop&w=2000&q=80";
                upload_to_firestore();
                return;
            }
